...
import { apiService, type CheckCodeResponse, type GameSessionInfo } from "../../service/api/api"

// Async thunks
export const checkOrderCode = createAsyncThunk(
  "gameSession/checkCode",
  async ({ uniqueCode, sellerId, captcha }: { uniqueCode: string; sellerId?: string; captcha?: string }) => {
    const response = await apiService.checkCode(uniqueCode, sellerId, captcha)
    return response
  },
)

export const confirmSending = createAsyncThunk("gameSession/confirmSending", async (uniqueCode: string) => {
  const response = await apiService.confirmSending(uniqueCode)
  return response
})

export const resetSteamAccount = createAsyncThunk("gameSession/resetSteamAccount", async (uniqueCode: string) => {
  const response = await apiService.resetSteamAccount(uniqueCode)
  return response
})

export const resetBot = createAsyncThunk("gameSession/resetBot", async (uniqueCode: string) => {
  const response = await apiService.resetBot(uniqueCode)
  return response
})

export const checkFriend = createAsyncThunk("gameSession/checkFriend", async (uniqueCode: string) => {
...
export const changeSteamContact = createAsyncThunk(
  ...
  async ({ uniqueCode, steamContact }: { uniqueCode: string; steamContact: string }) => {
...
export const { setUniqueCode, setSellerId, setCurrentStep, nextStep, resetError, clearSession } =
