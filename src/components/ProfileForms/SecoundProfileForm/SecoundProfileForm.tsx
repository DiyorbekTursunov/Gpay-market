...
const SecoundProfileForm: React.FC<OrderFormProps> = ({
  onSubmit,
  isLoading,
  error,
}) => {
...
//   };
  ...
  return (
    <form className="secound-order-form" onSubmit={handleSubmit}>
      {localError && <div className="secound-order-form__error">{localError}</div>}
      ...
      {error && (
        <div className="secound-order-form__error">{t("profileForm.robotError")}</div>
      )}
    </form>
  );
};
